...
compatible_urllist = ['.megaupload.com/','http://77.247.181.97/','.humexico.org/']
...
def get(url,usera,passa,cookiepath=None):
        ...
        if url_for_cookies(url) == True:
                ...
                if os.path.exists(cookiepath):
                    try:
                        ...
                        url=url.replace('premium','premium1')
                        print "ejecutando especial"
                        if e.code == 401:
                            cj = cookielib.LWPCookieJar()
                            cj.load(cookiepath)
                            user = usera
                ...
                else: return _loadwithoutcookies(url,usera,passa)                
        else: return _loadwithoutcookies(url)    
    else: return _loadwithoutcookies(url)

def _loadwithoutcookies(url,usera,passa):
    try:
        user = usera
        passw = passa
        password_mgr = urllib2.HTTPPasswordMgrWithDefaultRealm()
